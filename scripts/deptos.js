document.addEventListener("DOMContentLoaded", () => {
  ...
  const inputId = document.getElementById("modalIdDepto");
  const inputIsNew = document.getElementById("isNew");
  const inputImagenActual = document.getElementById("imagenActual");
  const inputImagenFile = document.getElementById("deptoImagen"); // id real del file
  ...
  function showDepto(index) {
    ...
    if (isAdmin && formDepto) {
      if (inputId) inputId.value = dep.iddepto || "";
      ...
      if (inputImagenActual) inputImagenActual.value = dep.imagen || ""; 
  ...
  if (btnNuevo) {
    if (isAdmin) {
      btnNuevo.addEventListener("click", () => {
        ...
        if (inputImagenActual) inputImagenActual.value = "";
        if (inputImagenFile) inputImagenFile.value = ""; 
...
fetch("/cass/auth/get_departamentos.php")
  ...
  .then((data) => {
    ...
    departamentos.forEach((dep, index) => {
      ...
      col.innerHTML = `
        <div class="card h-100 shadow-sm card-dep text-center" data-index="${dep.iddepto}">
    ...
    document.querySelectorAll(".card-dep").forEach((card, idx) => {
      card.addEventListener("click", () => {
        const index = idx; // usamos el índice de la iteración
        showDepto(index);
        bsModal.show();
      });
    });
